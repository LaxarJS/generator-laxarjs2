...
const LICENSE_UNLICENSED = 'UNLICENSED';
...
function createBanner( generator ) {
   ...
   return '/**\n * ' + [
      ...
      ( vars.license && vars.license !== LICENSE_UNLICENSED ) ?
         `Released under the ${vars.license} license` : null,
      vars.homepage
   ].filter( _ => _ ).join( '\n * ' ) + '\n */';
}
...
function determineArtifactDirName( generator, artifactType ) {
   let defaultPath = path.join( 'application', artifactType );
   const laxarConfigPath = generator.destinationPath( 'laxar.config.js' );
   if( generator.fs.exists( laxarConfigPath ) ) {
      const laxarConfig = require( laxarConfigPath );
      if( laxarConfig.paths && typeof laxarConfig.paths[ artifactType ] === 'string' ) {
         defaultPath = laxarConfig.paths[ artifactType ];
      }
   }
